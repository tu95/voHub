...
`)
	...
	err := AddDeviceInFile(path, DeviceConfig{
		...
		QMIProxyExecutable: "/opt/vohub/bin/qmi-proxy",
	})
	if err != nil {
	...
	if dev.QMIProxyExecutable != "/opt/vohub/bin/qmi-proxy" {
		t.Fatalf("QMIProxyExecutable=%q, want /opt/vohub/bin/qmi-proxy", dev.QMIProxyExecutable)
	}
}
