...
func TestProxyActionsReturnNotFoundForMissingInstance(t *testing.T) {
	gin.SetMode(gin.TestMode)
	tests := []struct {
		name    string
		handler func(*Server, *gin.Context)
	}{
		{name: "start", handler: func(s *Server, c *gin.Context) { s.handleProxyInstanceStart(c) }},
		{name: "restart", handler: func(s *Server, c *gin.Context) { s.handleProxyInstanceRestart(c) }},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx.Params = gin.Params{{Key: "instance_id", Value: "missing"}}
			ctx.Request = httptest.NewRequest(http.MethodPost, "/", nil)
			tt.handler(&Server{proxyMgr: proxyserver.NewManager()}, ctx)
...
func TestBuildProxyConfigsAllowsMissingRuntimeInterface(t *testing.T) {
	const missingIface = "vohub-missing-proxy-iface"
	initProxyTestConfig(t, `devices:
  - id: dev-missing
