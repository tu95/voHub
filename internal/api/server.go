...
import (
	...
	"github.com/tu95/vohub/internal/data/repo"
	"github.com/tu95/vohub/internal/db"
	"github.com/tu95/vohub/internal/device"
	"github.com/tu95/vohub/internal/global"
	"github.com/tu95/vohub/internal/notify"
	"github.com/tu95/vohub/internal/proxy/server"
	proxytraffic "github.com/tu95/vohub/internal/proxy/traffic"
	vwebsheet "github.com/tu95/vohub/internal/websheet"
...
type Server struct {
	...
	// Destructive or externally-networked system operations are injectable so
	// route tests can exercise the real handlers without modifying the host.
	checkUpdateFn checkUpdateFunc
	applyUpdateFn func() error
	uninstallFn   func()
}
...
func (s *Server) newRouter() *gin.Engine {
	...
	if s.cfg.Debug {
		r.GET("/debug/embed", s.authMiddleware(), func(c *gin.Context) {
			...
			testFiles := []string{"index.html", "assets", "favicon.svg"}
	...
	api.Use(s.authMiddleware())
	{
		...
		api.POST("/device-mgmt/discovered/fix-usbnet", s.handleFixDiscoveredUSBNet)            // 修复已发现设备 USBNET 模式
