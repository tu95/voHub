...
func (s *Server) handleProxyInstanceStart(c *gin.Context) {
	...
	if err := s.proxyMgr.Start(ctx, id); err != nil {
		c.JSON(proxyActionErrorStatus(err), gin.H{"status": "error", "message": err.Error()})
		return
	}
...
func proxyActionErrorStatus(err error) int {
	if errors.Is(err, server.ErrInstanceNotFound) {
