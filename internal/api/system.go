...
func detectServiceStopCommands(lookPath func(string) (string, error), statFile func(string) bool) [][]string {
	var cmds [][]string
	if statFile("/etc/init.d/vohub") {
		cmds = append(cmds, []string{"/etc/init.d/vohub", "disable"})
		cmds = append(cmds, []string{"/etc/init.d/vohub", "stop"})
		return cmds
	}
	if _, err := lookPath("systemctl"); err == nil {
		cmds = append(cmds, []string{"systemctl", "disable", "--now", "vohub"})
	}
	return cmds
}
