(function(){
  ...
  const shellBroadcastName = "vohub-websheet";
  const shellStorageKey = "vohub-websheet-complete";
    ...
    try {
      ...
      if (originalOpen) {
        window.XMLHttpRequest.prototype.open = function(method, url) {
          ...
          this.__vohubMethod = String(method || "GET").toUpperCase();
          this.__vohubURL = String(url || "");
        ...
        window.XMLHttpRequest.prototype.send = function() {
          try {
            this.addEventListener("load", function() {
              inspectATTAddressResponse(this.__vohubMethod || "GET", this.__vohubURL || "", this.responseText || "");
            });
          } catch (_) {}
  ...
  function shellMessage(payload) {
    return {type:"vohub-websheet-callback", token:websheetToken || "", callback: payload};
  }
