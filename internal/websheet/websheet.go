...
func callbackHTML() string {
	return `<!doctype html><html><body><script>try{window.parent.postMessage({type:"vohub-websheet-callback"},"*")}catch(_){}</script>Carrier flow returned to voHub.</body></html>`
}
