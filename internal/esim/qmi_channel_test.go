...
import (
	...
	"github.com/tu95/vohub/pkg/mbim"
)
...
func TestNewQMIUIMTransportWithOptionsStoresClientOptions(t *testing.T) {
	transport := NewQMIUIMTransportWithOptions("/dev/cdc-wdm0", qmiq.ClientOptions{
		...
		ProxyExecutable: "/opt/vohub/bin/qmi-proxy",
	})
	...
	if transport.clientOptions.ProxyExecutable != "/opt/vohub/bin/qmi-proxy" {
		t.Fatalf("ProxyExecutable=%q, want /opt/vohub/bin/qmi-proxy", transport.clientOptions.ProxyExecutable)
	}
