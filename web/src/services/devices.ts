...
function parseEsimBusyError(err: unknown): EsimBusyErrorMeta {
  ...
  if (!data || typeof data !== "object")
    return { busy: false, retryAfterMs: 0 };
...
async function withEsimBusyRetry<T>(fn: () => Promise<T>) {
  ...
  throw lastErr instanceof Error
    ? lastErr
    : new Error("eSIM operation failed after retries");
}
...
export const devicesService = {
  listManaged(signal?: AbortSignal) {
    return callService(async () => {
      ...
      return {
        ...
        deviceLimit: (typeof res.data?.device_limit === "number"
          ? res.data.device_limit
          : 0) as number,
      };
    });
  },
  ...
  listDiscovered() {
    return callService(async () => {
      const res = await api.get("/devices/discovered", {
        params: { with_imei: 1 },
      });
  ...
  setFlightMode(id: string, flightModeEnabled: boolean) {
    return callService(async () => {
      const res = await api.patch<FlightModeResponse>(
        `/devices/${id}/flight-mode`,
        { enabled: flightModeEnabled },
      );
      return {
  ...
  startNetwork(id: string, opts?: { ip_version?: string; apn?: string }) {
    return callService(async () => {
      const res = await api.patch<NetworkControlResponse>(
        `/devices/${id}/network`,
        {
          enabled: true,
  ...
  startE911Websheet(id: string) {
    return callService(async () => {
      const res = await api.post<CarrierWebsheetInfo>(
        `/devices/${id}/vowifi/e911/websheet`,
      );
      return res.data;
    });
  },
  ...
  updateConfig(id: string, config: DeviceConfigDTO) {
    return callService(async () => {
      const res = await api.put<UpdateConfigResponse>(`/devices/${id}`, {
        config,
      });
      return {
  ...
  addManaged(config: DeviceConfigDTO) {
    return callService(async () => {
      ...
      const res = await api.post<AddDeviceResponse>("/devices", {
        config: deviceConfig,
      });
      return {
        warning: typeof res.data?.warning === "string" ? res.data.warning : "",
  ...
  sendAT(id: string, payload: { cmd: string; timeout_ms: number }) {
    return callService(async () => {
      const res = await api.post<AtCommandResponse>(
        `/devices/${id}/actions/at`,
        payload,
      );
      return {
        ok: res.data?.ok ?? true,
  ...
  sendUSSD(
    id: string,
    payload: { command: string; timeout_ms: number },
    timeoutMs: number,
  ) {
    return callService(async () => {
      const res = await api.post<UssdResponse>(
        `/devices/${id}/actions/ussd`,
        payload,
        {
          timeout: timeoutMs,
        },
  ...
  continueUSSD(
    id: string,
    payload: { session_id: string; input: string; timeout_ms: number },
    timeoutMs: number,
  ) {
    return callService(async () => {
      const res = await api.post<UssdResponse>(
        `/devices/${id}/actions/ussd/continue`,
        payload,
        {
          timeout: timeoutMs,
        },
  ...
  cancelUSSD(id: string, sessionId: string) {
    return callService(async () => {
      await api.post(`/devices/${id}/actions/ussd/cancel`, {
  ...
  getEsimOverview(
    ...
    return callService(async () => {
      const res = await withEsimBusyRetry(() =>
        api.get<EsimOverviewResponse>(`/devices/${id}/esim`, {
  ...
  getEsimProfiles(
    ...
    return callService(async () => {
      const res = await withEsimBusyRetry(() =>
        api.get(`/devices/${id}/esim/profiles`, {
          signal: options?.signal,
          params: options?.refresh ? { refresh: true } : undefined,
        }),
      );
  ...
  getEsimNotifications(
    id: string,
    options?: { aidHex?: string; signal?: AbortSignal },
  ) {
    return callService(async () => {
      const res = await withEsimBusyRetry(() =>
        api.get<{ items?: EsimNotificationItem[] }>(
          `/devices/${id}/esim/notifications`,
          {
            signal: options?.signal,
    ...
    return callService(async () => {
      const res = await withEsimBusyRetry(() =>
        api.post<{ status?: string; message?: string }>(
          `/devices/${id}/esim/notifications/${sequenceNumber}/actions/retry`,
          null,
          {
  ...
  switchEsimProfile(id: string, payload: { iccid: string; aid_hex: string }) {
    return callService(async () => {
      await withEsimBusyRetry(() =>
        api.post(`/devices/${id}/esim/actions/switch`, payload),
      );
      return true;
    });
  },
  renameEsimProfile(
    id: string,
    iccid: string,
    payload: { name: string; aid_hex: string },
  ) {
    return callService(async () => {
      await withEsimBusyRetry(() =>
        api.patch(`/devices/${id}/esim/profiles/${iccid}`, payload),
      );
      return true;
    });
  },
  deleteEsimProfile(id: string, iccid: string, aidHex: string) {
    return callService(async () => {
      const res = await withEsimBusyRetry(() =>
        api.delete<DeleteEsimProfileResponse>(
          `/devices/${id}/esim/profiles/${iccid}`,
          {
      ...
      return {
        warning: typeof res.data?.warning === "string" ? res.data.warning : "",
        warningCode:
          typeof res.data?.warning_code === "string"
            ? res.data.warning_code
            : "",
  ...
  downloadEsimProfile(
    ...
    return callService(async () => {
      await api.get(`/devices/${id}/esim/actions/download`, {
        params: payload,
      });
      return true;
    });
  },
  downloadEsimProfileStream(
    id: string,
    payload: {
      smdp: string;
      matching_id?: string;
    ...
    onEvent: (event: EsimDownloadEvent) => void,
    signal?: AbortSignal,
  ) {
    return callService(async () => {
      ...
      const response = await fetch(
        `${api.defaults.baseURL || ""}/devices/${id}/esim/actions/download?${params}`,
        {
          headers: {
      ...
      if (!response.body) throw new Error("eSIM 下载事件流为空");

      const reader = response.body.getReader();
      const decoder = new TextDecoder();
      let buffer = "";
      let terminal: EsimDownloadEvent | undefined;
      while (!terminal) {
        ...
        while (newline >= 0) {
          ...
          if (line.startsWith("data:")) {
            const event = JSON.parse(line.slice(5).trim()) as EsimDownloadEvent;
            onEvent(event);
      ...
      if (!terminal) throw new Error("eSIM 下载事件流提前结束");
      if (terminal.step === "error") {
        throw new Error(
          terminal.code === "euicc_insufficient_memory"
            ? "eUICC 空间不足，请删除未使用的 Profile 后重试"
            : terminal.msg || "eSIM 下载失败",
        );
      }
      return terminal;
    });
  ...
  scanOperators(id: string, signal?: AbortSignal) {
    return callService(async () => {
      const res = await api.get(`/devices/${id}/operator_selection/scan`, {
        signal,
      });
  ...
  setOperatorSelection(
    id: string,
    payload: import("../types/api").SetOperatorSelectionRequest,
  ) {
    return callService(async () => {
