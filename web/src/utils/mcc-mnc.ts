...
export function lookupServingOperatorNameFromPLMN(
  index: Map<string, MccMncRow>,
  modem: ServingOperatorLike,
): MccMncRow | null {
...
export function formatServingOperatorDisplay(
  modem: ServingOperatorLike,
  index: Map<string, MccMncRow> | null,
): string {
  const op = normalizeCode(modem?.operator || "");
  if (!index) return op || "--";
